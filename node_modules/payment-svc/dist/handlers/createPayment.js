 ...
 */
async function handleCreatePayment(request, paymentProvider, supabase) {
    try {
        const { invoice_id, organization_id, customer_id, amount, currency, description, customer_email, metadata } = request;
        logger_1.logger.info(`Creating payment for invoice ${invoice_id} (${amount} ${currency})`);
        ...
        const { data: invoice, error: fetchError } = await supabase
            ...
            .single();
        if (fetchError) {
            logger_1.logger.error(`Error fetching invoice ${invoice_id}: ${fetchError.message}`);
        ...
        if (invoice.status === 'paid') {
            logger_1.logger.warn(`Invoice ${invoice_id} is already paid`);
        ...
        if (invoice.total_amount !== amount) {
            logger_1.logger.warn(`Payment amount (${amount}) does not match invoice amount (${invoice.total_amount})`);
        }
        ...
        if (insertError) {
            logger_1.logger.error(`Error storing payment record: ${insertError.message}`);
        ...
        const { error: updateError } = await supabase
            ...
            .eq('organization_id', organization_id);
        if (updateError) {
            logger_1.logger.warn(`Error updating invoice status: ${updateError.message}`);
            // Non-critical error, continue
        }
        logger_1.logger.info(`Payment session created for invoice ${invoice_id}, payment ID: ${payment_id}`);
        return {
            payment_id,
        ...
        logger_1.logger.error(`Error in handleCreatePayment: ${error instanceof Error ? error.message : String(error)}`);
        throw error;
    }
}
