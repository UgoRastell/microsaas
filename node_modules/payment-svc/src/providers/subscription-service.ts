...
export class SubscriptionService {
  ...
  private postalDeliveryPricePerInvoice: number = 0.30;
   ...
   */
  async initializeStripePlans(): Promise<SubscriptionPlan[]> {
    try {
      logger.info('Initialisation des plans dans Stripe...');
      ...
      for (const plan of this.plans) {
        ...
        try {
          ...
          logger.info(`Produit existant pour ${plan.name}: ${product.id}`);
        } catch (error) {
          ...
          logger.info(`Nouveau produit créé pour ${plan.name}: ${product.id}`);
        }
        ...
        try {
          ...
          price = prices.data[0];
          if (!price) throw new Error('Aucun prix trouvé');
          logger.info(`Prix existant pour ${plan.name}: ${price.id}`);
        } catch (error) {
          ...
          if (plan.price > 0) {
            ...
            logger.info(`Nouveau prix créé pour ${plan.name}: ${price.id}`);
          }
        }
      ...
      try {
        ...
        try {
          ...
          if (productExists) {
            logger.info(`Produit existant pour livraison postale: postal-delivery`);
          } else {
            ...
            logger.info(`Nouveau produit créé pour livraison postale: ${postalProductId}`);
          }
        } catch (error) {
          logger.error('Erreur lors de la création du produit d\'envoi postal');
        }
        
        logger.info('Initialisation des plans dans Stripe terminée avec succès');
        return this.plans;
      } catch (error) {
        logger.error('Erreur lors de la création du produit d\'envoi postal');
        return this.plans;
      ...
      logger.error(`Erreur lors de l'initialisation des plans Stripe: ${error instanceof Error ? error.message : String(error)}`);
      return this.plans;
    }
  }
   ...
   */
  async createSubscriptionCheckout(params: CreateSubscriptionParams) {
    try {
      ...
      logger.error(`Erreur lors de la création du checkout d'abonnement: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
   ...
   */
  private async createFreemiumSubscription(params: CreateSubscriptionParams) {
  ...
  async handleSubscriptionCreated(event: Stripe.Event) {
    ...
    if (!plan) {
      logger.warn(`Plan non trouvé pour l'abonnement: ${planId}`);
      return;
    }
    ...
    if (error) {
      logger.error(`Erreur lors de la création de l'abonnement dans Supabase: ${error.message}`);
      throw error;
    }
  ...
  async handleSubscriptionUpdated(event: Stripe.Event) {
    ...
    if (error) {
      logger.error(`Erreur lors de la mise à jour de l'abonnement dans Supabase: ${error.message}`);
      throw error;
    }
...
async getUserSubscription(userId: string) {
  logger.info(`Récupération de l'abonnement pour userId: ${userId}`);
  try {
    ...
    try {
      ...
      if (data) {
        logger.info(`Abonnement trouvé par user_id pour userId: ${userId}: ${data.id} (${data.plan_type})`);
        return data;
      }
    } catch (err) {
      logger.info(`Pas d'abonnement trouvé via user_id pour ${userId}, essai via customer_id`);
    }
    ...
    if (this.isValidUUID(userId)) {
      try {
        ...
        if (data) {
          logger.info(`Abonnement trouvé par customer_id pour userId: ${userId}: ${data.id} (${data.plan_type})`);
          return data;
        }
      } catch (err) {
        logger.info(`Pas d'abonnement trouvé via customer_id pour ${userId}`);
      }
    } else {
      logger.warn(`L'identifiant utilisateur ${userId} n'est pas un UUID valide, impossible de rechercher par customer_id`);
    }

    // Si toujours rien, retourner null
    logger.warn(`Aucun abonnement trouvé pour userId: ${userId}`);
    return null;
  } catch (error) {
    logger.error(`Erreur lors de la récupération de l'abonnement: ${error instanceof Error ? error.message : String(error)}`);
   ...
   */
  async incrementInvoiceUsage(organizationId: string, count: number = 1) {
    try {
      ...
      logger.error(`Erreur lors de l'incrémentation de l'utilisation des factures: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
}
