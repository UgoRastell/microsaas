...
async function handleSubscriptionEvent(
  ...
  try {
    ...
    logger.info(`Traitement de l'événement d'abonnement ${eventType} pour l'abonnement ${subscription.id}`);
    ...
    switch (eventType) {
      case 'customer.subscription.created':
        ...
        logger.info(`Abonnement créé et synchronisé: ${subscription.id}`);
        break;
      ...
      default:
        logger.info(`Événement d'abonnement non géré: ${eventType}`);
    }
    
    return { success: true };
  } catch (error) {
    logger.error(`Erreur lors du traitement de l'événement d'abonnement: ${error instanceof Error ? error.message : String(error)}`);
    throw error;
  }
}
